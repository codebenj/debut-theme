<?php
//activate
if (! function_exists('activate_linked_options_addon')) {
    function activate_linked_options_addon($StoreThemes, $shop) {
                ...
                $liveview_addon_schema = (string) '
{
    ...
    "settings": [
      ...
      {
        "type": "checkbox",
        "id": "dbtfy_linked_options",
        "label": "Activate",
        "info": "Only works with the dropdown variant picker type",
        "default": false
      }
    ]
  }';

                if( ( $badgePos = strpos( $schema , 'dbtfy_linked_options' ) ) === false ) {
                ...
                logger('update schema on linked options addon throws client exception');
            }
            catch(\Exception $e){
                logger('update schema on linked options addon throws exception');
            }
            ...
            try{
                ...
                {%- if section.settings.product_selector == "select" -%}
                <script>
                ...
                */
                ...
                $create_trustbadge_snippet = $shop->api()->request(
                    'PUT',
                    '/admin/api/themes/'.$theme->shopify_theme_id.'/assets.json',
                    ['asset' => ['key' => 'snippets/dbtfy-linked-options.liquid', 'value' => $snippet] ]
                );
            }
            catch(\GuzzleHttp\Exception\ClientException $e){
                logger('add linked options throws client exception');
            }
            catch(\Exception $e){
                logger('add linked options throws exception');
            }
            ...
            try{
                ...
                if( ( $pos = strpos( $product_template , 'dbtfy-linked-options' ) ) === false ) {
                    if( ( $pos = strrpos( $product_template , '</div><!-- /.product-single -->' ) ) !== false ) {
                        $new_prod_template = str_replace('</div><!-- /.product-single -->', '</div><!-- /.product-single -->{% include "dbtfy-linked-options" %}', $product_template);
                ...
                logger('update linked options include throws client exception');
            }
            catch(\Exception $e){
                logger('update linked options include throws exception');
            }
...
//deativate
if (! function_exists('deactivate_linked_options_addon')) {
    function deactivate_linked_options_addon($StoreThemes, $shop, $checkaddon) {
        foreach ($StoreThemes as $theme) {
            ...
            {
                deactivate_linked_options_addon_curl($theme, $shop);
                continue;
            }
            else
            {
            ...
            $json = array_filter($json, function ($obj) {
              if (stripos($obj['name'], 'Linked options') !== false) {
                  return false;
              }
              return true;
            });

            if(str_contains($schema,'Linked options')){
            ...
            $delete_trustbadge_snippet = $shop->api()->request(
                'DELETE',
                '/admin/api/themes/'.$theme->shopify_theme_id.'/assets.json',
                ['asset' => ['key' => 'snippets/dbtfy-linked-options.liquid'] ]
            );
            ...
            if(str_contains($product_template,'dbtfy-linked-options')){
                $value  =  explode('{% include "dbtfy-linked-options" %}',$product_template,2);
...
if (! function_exists('deactivate_linked_options_addon_curl')) {
  function deactivate_linked_options_addon_curl($theme, $shop) {
    ...
    // remove snippet
    $delete_trustbadge_snippet = deleteThemeFilesCurl($shop, $theme, 'snippets/dbtfy-linked-options.liquid');
