...
class Diff{
  ...
  public static function compare(
    ...
    while ($start <= $end1 && $start <= $end2
        && trim($sequence1[$start]) == trim($sequence2[$start])){
      $start ++;
    }

    // skip any common suffix
    while ($end1 >= $start && $end2 >= $start
        && trim($sequence1[$end1]) == trim($sequence2[$end2])){
      $end1 --;
      $end2 --;
    }
  ...
  private static function generatePartialDiff(
    ...
    while ($index1 > 0 || $index2 > 0){
      ...
      if ($index1 > 0 && $index2 > 0
          && trim($sequence1[$index1 + $start - 1])
              == trim($sequence2[$index2 + $start - 1])){
  ...
  /* Returns a Inserted as an array,
   *
   * $diff      - the diff array
   */
  public static function toInsertedArray($diff){
  ...
  /* Returns a Deleted as an array,
   *
   * $diff      - the diff array
   */
  public static function toDeletedArray($diff){
